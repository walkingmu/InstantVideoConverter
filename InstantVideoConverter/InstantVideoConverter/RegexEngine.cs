...
namespace InstantVideoConverter
{
    ...
    internal static class RegexEngine
    {
        ...
        internal static Metadata GetMetaData(string data)
        {
            Metadata mData = new Metadata();
            GetVideoMetadata(data, mData);
            GetAudioMetaData(data, mData);
            return mData;
        }

        internal static void GetAudioMetaData(string data, Metadata mData)
        {
            ...
            mData.AudioData = new Metadata.Audio
                {
        ...
        internal static bool GetVideoMetadata(string data, Metadata mData)
        {
            Match matchMetaVideo = Index[Find.MetaVideo].Match(data);
            if (!matchMetaVideo.Success) return false;
        ...
        /// </param>
        internal static ConvertProgressEventArgs IsProgressData(string data)
        {
            ...
            ConvertProgressEventArgs progressEventArgs = new ConvertProgressEventArgs(processedDuration, TimeSpan.Zero, frame, fps, sizeKb, bitrate);
        ...
        ///     <para>generated from the data. </para>
        /// </param>
        internal static ConversionCompleteEventArgs IsConvertCompleteData(string data)
        {
            Match matchFrame = Index[Find.ConvertProgressFrame].Match(data);
            Match matchFps = Index[Find.ConvertProgressFps].Match(data);
            ...
            if (!matchFrame.Success || !matchFps.Success || !matchFinished.Success || !matchTime.Success ||
                !matchBitrate.Success) return null;
            TimeSpan processedDuration;
            TimeSpan.TryParse(matchTime.Groups[1].Value, out processedDuration);
            ...
            ConversionCompleteEventArgs conversionCompleteEvent = new ConversionCompleteEventArgs(processedDuration, TimeSpan.Zero, frame, fps, sizeKb, bitrate);
            return conversionCompleteEvent;
        }
